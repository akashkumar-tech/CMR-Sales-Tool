...
export default function LeadDrawer({ leadId, open, onOpenChange, onChange, members = [], initialTab = "overview" }) {
  ...
  const [act, setAct] = useState({ type: "Call", contact_method: "Phone Call", notes: "", outcome: "", next_action: "", next_follow_up: "" });
  ...
  const logActivity = async () => {
    ...
    try {
      ...
      setAct({ type: "Call", contact_method: "Phone Call", notes: "", outcome: "", next_action: "", next_follow_up: "" });
      load(); onChange?.();
      ...
      <SheetContent className="bg-white w-full sm:max-w-xl overflow-y-auto p-0">
        ...
        <Tabs value={tab} onValueChange={setTab} className="p-6">
          ...
          <TabsContent value="log" className="mt-4 space-y-3">
            ...
            <div><Label>Next Follow-up Date</Label><Input type="date" value={act.next_follow_up} onChange={(e) => setAct((a) => ({ ...a, next_follow_up: e.target.value }))} /></div>
